...
# Replace all deimal points with commas in hints
...
for dir in "$perseus_dir"/*; do
    if [ -d "$dir"/images ];
    then
        for file in "$dir"/images/*; do
        if [ -f "$file" ] && [[ "$file" == *"-data.json" ]] && ! [[ "$file" == *"exercise.json" ]]; then
                    ...
                    sed -i -r 's/\\\\LARGE\{\.\}/\\\\LARGE\{\,\}/g' "$file"

                    sed -i -r 's/([[:digit:]]+)\.([[:digit:]]+)\\\\green/\1\,\2\\\\green/g' "$file"
                    sed -i -r 's/([[:digit:]]+)\.([[:digit:]]+)\\\\red/\1\,\2\\\\red/g' "$file"
                    sed -i -r 's/([[:digit:]]+)\.([[:digit:]]+)\\\\blue/\1\,\2\\\\blue/g' "$file"
